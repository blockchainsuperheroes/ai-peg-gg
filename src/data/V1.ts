...
export function useV1ExchangeAddress(tokenA?: string, tokenB?: string): string | undefined {
...
function useMockV1Pair(inputCurrency?: Currency): MockV1Pair | undefined {
  ...
  // Ensure the addresses are defined before using them
  const tokenAddress = token?.address
  const wethAddress = token ? WETH[token.chainId]?.address : undefined
  const v1PairAddress = useV1ExchangeAddress(isWETH ? wethAddress : tokenAddress, isWETH ? tokenAddress : wethAddress)
...
export function useAllTokenV1Exchanges(): { [exchangeAddress: string]: Token } {
  ...
  const data = useSingleContractMultipleData(factory, 'getPair', args, NEVER_RELOAD)
